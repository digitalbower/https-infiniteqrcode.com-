...
@section('title', 'Edit Bitcoin QRCodes')
@section('content')
      <!-- Main Content Area -->
                                  ...
                                  <div class="w-full p-3 mt-2 border border-gray-300 rounded-lg shadow-sm focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500">                           
                                    ...
                                    <div class="relative">
                                        <button type="button" id="folderDropdownButton"
                                            ...
                                            <span id="selectedFolder">{{ $bitcoin->folder_name ?? 'Select a folder' }}</span>
                                        ...
                                        <div id="folderDropdown" class="hidden absolute z-10 w-full bg-white border border-gray-300 rounded shadow mt-1">
                                            ...
                                            <ul id="folderList" class="divide-y divide-gray-200">
                                                @foreach ($folders as $folder)
                                                    @php
                                                        $isSelected = isset($bitcoin->folder_name) && $bitcoin->folder_name == $folder->name ? 'bg-gray-200 font-bold' : '';
                                                    @endphp
                                    ...
                                    <input id="folderinput" type="hidden" name="folderinput" value="{{ $bitcoin->folder_name ?? '' }}" />
                                  </div>                            
...
<script>
    $(document).ready(function () {
      ...
      $(".address").val(address);
      var network = $("#btcnetwrk").attr('value'); 
      $(".network").val(network);
